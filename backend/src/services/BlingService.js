...
class BlingService {
  ...
  getAuthorizationUrl() {
    ...
    const scope =
      "vendas:write vendas:read produtos:read contatos:read notas_fiscais:write";
  ...
  async getAccessToken() {
    try {
      ...
      if (error || !integration) {
        throw new Error(
          "Bling integration not found. Please authenticate first.",
        );
      }

      const lastUpdate = new Date(integration.updated_at);
      const expireTime = new Date(
        lastUpdate.getTime() + integration.expires_in * 1000,
      );
      const now = new Date();
  ...
  async createSalesOrder(orderId) {
    try {
      console.log(
        `📡 Iniciando processamento de pedido de venda no Bling para o Pedido ID: ${orderId}`,
      );
      ...
      if (profileError || !profile) {
        throw new Error(
          `Perfil do utilizador ${order.user_id} não encontrado.`,
        );
      }
          ...
          if (contactRes.data?.data?.length > 0) {
            ...
            if (address) {
              try {
                const updateContactPayload = {
                  ...
                  endereco: {
                    geral: {
                      ...
                      municipio: (address.cidade || "São Paulo").substring(
                        0,
                        50,
                      ),
                ...
                console.log(
                  `   ✅ Endereço do contato atualizado no Bling com sucesso.`,
                );
              } catch (updateErr) {
                console.log(
                  "   ⚠️ Erro ao atualizar endereço do contato (ignorado):",
            ...
            try {
              ...
              console.log(
                `   ✅ Novo contato CRIADO no Bling: ${blingContactId}`,
              );
            } catch (createErr) {
              console.log(
                "   ⚠️ Erro ao criar contato com endereço:",
      ...
      console.log(
        `✅ Pedido de venda criado com sucesso no Bling! ID: ${blingOrderId}`,
      );
  ...
  async emitirNFe(internalOrderId, blingOrderId) {
    try {
      console.log(
        `📡 Solicitando emissão de NF-e para o Pedido Bling: ${blingOrderId} (Interno: ${internalOrderId})`,
      );
      const token = await this.getAccessToken();
      ...
      const response = await axios.post(
        `${BLING_API_URL}/pedidos/vendas/${blingOrderId}/gerar-nfe`,
        {},
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      ...
      const nfeId =
        response.data?.data?.idNotaFiscal || response.data?.data?.id;
      if (!nfeId)
        throw new Error("ID da Nota Fiscal não retornado pelo Bling.");
      ...
      try {
        ...
        console.log(
          "📄 Detalhes da Nota Fiscal recebidos:",
        ...
        const chaveAcesso =
          ...
          nfeData.notaFiscal?.chaveAcesso;
        const linkDanfe =
          nfeData.linkDanfe ||
          nfeData.link_danfe ||
          nfeData.transporte?.linkDanfe ||
          nfeData.retorno?.linkDanfe;
        ...
        console.log(
          `💾 Pedido ${internalOrderId} atualizado com Chave: ${chaveAcesso} e Link: ${linkDanfe ? "Sim" : "Não"}`,
        );
      } catch (detailError) {
        console.warn(
          "⚠️ Nota gerada, mas erro ao buscar detalhes extras (Link/Chave):",
          detailError.message,
        );
  ...
  async getProducts() {
    ...
    try {
      ...
      console.error(
        "Error fetching products from Bling:",
        error.response?.data || error.message,
      );
      throw error;
    }
  ...
  async createProduct(productData) {
    ...
    try {
      ...
      console.error(
        "Error creating product in Bling:",
        error.response?.data || error.message,
      );
      throw error;
    }
